...
def storage_health_check(**context):
    ...
    from plugins.services.monitoring_service import (
        run_health_check,
    ...
    try:
        ...
        print(
            f"開始存儲健康檢查 - {taipei_now.format('YYYY-MM-DD HH:mm:ss')} (台北時間)"
        )
        ...
        if not health_report.is_healthy:
            ...
            raise RuntimeError(
                "Storage is unhealthy: " + "; ".join(health_report.issues)
            )

        return {
            "health_status": (
                "healthy" if health_report.is_healthy else "issues_detected"
            ),
            "total_items": health_report.total_items,
            "total_size_mb": health_report.total_size_mb,
...
def check_storage_capacity(**context):
    ...
    try:
        ...
        if storage_stats.get("error") or not storage_stats.get(
            "redis_connected", False
        ):
            raise RuntimeError(
                f"Storage statistics unavailable: {storage_stats.get('error')}"
            )
