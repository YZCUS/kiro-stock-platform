...
def storage_maintenance(**context):
    ...
    try:
        ...
        print(
            f"開始存儲維護任務 - {taipei_now.format('YYYY-MM-DD HH:mm:ss')} (台北時間)"
        )
        ...
        print(
            f"  - 清理過期項目: {maintenance_result['summary'].get('expired_items_cleaned', 0)}"
        )
        print(
            f"  - 清理指標數據: {maintenance_result['summary'].get('metrics_cleaned', 0)}"
        )
        print(
            f"  - 整體健康: {'是' if maintenance_result['summary'].get('overall_health') else '否'}"
        )
        ...
        if maintenance_result["tasks_failed"]:
            ...
            for failed_task in maintenance_result["tasks_failed"]:
                print(
                    f"  - {failed_task['task']}: {failed_task.get('error', 'Unknown error')}"
                )

        if maintenance_result["tasks_failed"] or not maintenance_result["summary"].get(
            "overall_health", False
        ):
            raise RuntimeError("Storage maintenance did not finish healthy")
