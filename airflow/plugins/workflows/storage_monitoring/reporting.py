...
def generate_storage_report(**context):
    ...
    try:
        ...
        if not health_report.is_healthy:
            raise RuntimeError(
                "Cannot publish a healthy storage report: "
                + "; ".join(health_report.issues)
            )
        if performance_metrics.get("error"):
            raise RuntimeError(
                f"Storage performance metrics unavailable: "
        ...
        report = {
            ...
            "performance_summary": {
                "total_operations": performance_metrics.get("summary", {}).get(
                    "total_operations", 0
                ),
                "success_rate_percent": performance_metrics.get("summary", {}).get(
                    "success_rate_percent", 0
                ),
                "failure_rate_percent": performance_metrics.get("summary", {}).get(
        ...
        print(
            f"  - 成功率: {performance_metrics.get('summary', {}).get('success_rate_percent', 0):.1f}%"
        )
