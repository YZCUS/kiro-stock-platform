...
def _run_market_data_pipeline_with_market(context, market):
    """Run canonical collect/aggregate/validate for one complete market."""
    from plugins.operators.api_operator import APICallOperator

    print(f"啟動 {market} 多 timeframe market data pipeline...")
    operator = APICallOperator(
        task_id=f"run_market_data_pipeline_{market.lower()}_api",
    ...
    print(f"{market} 多 timeframe pipeline 完成")
    return result


def run_market_data_pipeline_tw(**context):
    return _run_market_data_pipeline_with_market(context, "TW")


def run_market_data_pipeline_us(**context):
    return _run_market_data_pipeline_with_market(context, "US")
...
def try_main_collection_workflow(**context):
    ...
    try:
        ...
        get_stocks_operator = APICallOperator(
            task_id=f"main_get_stocks", endpoint="/stocks/active", method="GET"
        )
        ...
        try:
            ...
            collect_operator = APICallOperator(
                ...
                method="POST",
                payload={"stocks": formatted_stocks, "use_stock_list": True},
            )
        ...
        # 處理外部存儲的情況
        if isinstance(collection_result, dict) and collection_result.get(
            "external_storage"
        ):
            from plugins.services.storage_service import retrieve_large_data
        ...
        def determine_api_success(data, has_stats):
            ...
            if has_stats:
                ...
                if total_stocks > 0 or success_count > 0:
                    return (
                        True,
                        f"成功處理 {total_stocks} 支股票（成功: {success_count}）",
                    )
                elif total_stocks == 0 and success_count == 0:
                    # 統計欄位存在但全為0，可能是空結果
                    return (
                        False,
                        f"無數據收集（total_stocks={total_stocks}, success_count={success_count}）",
                    )
            ...
            for field in no_data_indicators:
                if field in data:
                    ...
                    has_negative_indicator = any(
                        pattern in msg for pattern in negative_patterns
                    )
                    if has_negative_indicator:
                        ...
                        is_success_message = any(
                            pattern in msg for pattern in success_patterns
                        )
                    ...
                    has_no_data_indicator = any(
                        pattern in msg for pattern in no_data_patterns
                    )
        ...
        # 使用強健的成功判斷
        has_meaningful_stats = (
            total_stocks > 0
            or success_count > 0
            or "total_stocks" in collection_data
            or "success_count" in collection_data
        )
        api_success, success_reason = determine_api_success(
            collection_data, has_meaningful_stats
        )
        ...
        if collection_strategy == "collect_all_fallback":
            ...
            if isinstance(collection_data, list):
                ...
                if missing_stats:
                    strategy_components.append(
                        f"部分統計不可用({', '.join(missing_stats)})"
                    )

            # 添加市場範圍說明
            if actual_stocks_fetched != stocks_count:
                strategy_components.append(
                    f"實際範圍{actual_stocks_fetched}支 vs 活躍股票{stocks_count}支"
                )
            ...
            strategy_info = (
                f"主要策略(collect-batch): 基於活躍股票清單({stocks_count}支)"
            )
        ...
        if actual_stocks_fetched is None:
            ...
            print(
                f"警告：actual_stocks_fetched 為 None，已設為 {actual_stocks_fetched}"
            )
        ...
        notification_context = {
            ...
            "statistics_reliability": (
                "estimated" if isinstance(collection_data, list) else "api_provided"
            ),
        }
        ...
        if collection_strategy == "collect_all_fallback":
            ...
            if isinstance(collection_data, list):
                notification_context["note"] = (
                    f"統計數據基於list長度({len(collection_data)})推算"
                )
        ...
        return {
            ...
            "api_message": (
                api_message[:200] if api_message else ""
            ),  # 截斷消息以避免過長
            # 保留原始結果引用以便清理（外部存儲情況）
            "original_result": (
                collection_result
                if isinstance(collection_result, dict)
                and collection_result.get("external_storage")
                else None
            ),
        }
...
def _extract_collection_statistics(collection_data, market_name=""):
    ...
    """
    if isinstance(collection_data, dict):
        total_stocks = collection_data.get(
            "total_stocks", collection_data.get("processed", 0)
        )
        success_count = collection_data.get(
            "success_count", collection_data.get("successful", 0)
        )
        error_count = collection_data.get(
            "error_count", collection_data.get("failed", 0)
        )
        total_data_saved = collection_data.get(
            "total_data_saved",
            collection_data.get(
                "total_records", collection_data.get("records_collected", 0)
            ),
        )
        api_message = collection_data.get("message", "")
    elif isinstance(collection_data, list):
...
def _try_main_collection_with_market(context, market, market_name):
    ...
    # 轉換股票清單格式
    formatted_stocks = [
        {"symbol": stock.get("symbol"), "market": market} for stock in stocks_list
    ]
    ...
    print(
        f"{market_name}主要收集完成: 目標 {actual_stocks_fetched} 支，實際收集 {total_stocks} 支"
    )
    print(f"API成功判斷: {api_success} - {success_reason}")

    if not api_success:
        error_msg = (
            f"{market_name}API收集失敗: {success_reason} | 原始消息: {api_message}"
        )
        print(error_msg)
        raise Exception(error_msg)
...
def determine_api_success(data, has_stats):
    ...
    if "error" in data or "errors" in data:
        ...
        if errors:  # 有錯誤內容
            error_msg = (
                str(errors) if not isinstance(errors, list) else f"{len(errors)} errors"
            )
            return False, f"檢測到錯誤: {error_msg}"
