...
def context_interval_date(context: dict, timezone: str) -> date:
    """Return the scheduled interval end date in the market timezone."""
    ...
    if value is None:
        return pendulum.now(timezone).date()
    if not isinstance(value, pendulum.DateTime):
        value = pendulum.instance(value)
    return value.in_timezone(timezone).date()
...
def is_market_hours(
    market: str = "TW", check_time: Optional[pendulum.DateTime] = None
) -> bool:
    """
    檢查是否在市場交易時間內
    ...
    if market == "TW":
        ...
        return current_time >= pendulum.time(21, 30) or current_time <= pendulum.time(
            4, 0
        )
    else:
        # 預設為工作時間
