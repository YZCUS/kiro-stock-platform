...
async def generate_buy_sell_points_command(
    stock_symbols: List[str], days: int = 30, min_confidence: float = 0.6
):
    """生成買賣點命令"""
                ...
                if result.success:
                    ...
                    if result.buy_sell_points:
                        ...
                        for i, point in enumerate(
                            result.buy_sell_points[:5]
                        ):  # 只顯示前5個
                            action_text = (
                                "買入" if point.action == BuySellAction.BUY else "賣出"
                            )
                            print(
                                f"    {i+1}. {point.date} | {action_text} | {point.priority.value}"
                            )
                            print(
                                f"       價格: {point.price:.2f} | 信心度: {point.confidence:.3f}"
                            )
                            ...
                            if point.stop_loss and point.take_profit:
                                print(
                                    f"       止損: {point.stop_loss:.2f} | 止盈: {point.take_profit:.2f}"
                                )
                else:
...
async def analyze_market_trend_command(stock_symbols: List[str], days: int = 30):
    """分析市場趨勢命令"""
    ...
    try:
        async with get_db() as db_session:
            ...
            for symbol in stock_symbols:
                ...
                # 生成買賣點分析
                result = await buy_sell_signal_generator.generate_buy_sell_points(
                    db_session, stock_id=stock.id, days=days, min_confidence=0.5
                )
            ...
            for symbol in stock_symbols:
                ...
                if result.success:
                    for point in result.buy_sell_points:
                        ...
                        if priority_levels.get(point.priority, 0) < priority_levels.get(
                            min_priority_enum, 0
                        ):
                            continue
...
def main():
    ...
    filter_parser.add_argument(
        "--min-confidence", type=float, default=0.7, help="最小信心度"
    )
    ...
    notify_parser.add_argument(
        "--min-confidence", type=float, default=0.8, help="最小信心度"
    )
    notify_parser.add_argument(
        "--min-priority",
        choices=["low", "medium", "high", "critical"],
    ...
    if args.command == "generate":
        asyncio.run(
            generate_buy_sell_points_command(
        ...
        asyncio.run(
            analyze_market_trend_command(stock_symbols=args.symbols, days=args.days)
        )

    elif args.command == "filter":
        asyncio.run(
            filter_signals_command(
                stock_symbols=args.symbols,
                action=args.action,
