...
async def data_integrity_command(stock_symbols: List[str], days: int = 30):
    """數據完整性檢查命令"""
    try:
        await redis_client.connect()
...
def main():
    ...
    cache_parser.add_argument(
        "action", choices=["warm_up", "clear", "stats"], help="快取操作"
    )
    ...
    if args.command == "calculate":
        ...
        asyncio.run(data_integrity_command(stock_symbols=args.symbols, days=args.days))
