...
class TechnicalAnalysisCLI:
    ...
    async def list_stocks(self, market: str = None, limit: int = 20):
        ...
        async with self.async_session_local() as session:
            ...
            if market:
                stocks, _ = await stock_repo.get_multi_with_filter(
            ...
            for stock in stocks:
                print(
                    f"{stock.id:<5} {stock.symbol:<12} {stock.market:<6} {stock.name or 'N/A':<20}"
                )

    async def calculate_indicators(
        self, stock_id: int, indicators: str = "all", days: int = 100, save: bool = True
    ):
        """計算技術指標"""
        async with self.async_session_local() as session:
            # ✅ Clean Architecture: 實例化 repository
            ...
            if indicators == "all":
                ...
                indicator_names = [
                    name.strip().upper() for name in indicators.split(",")
                ]
                indicator_list = []
    ...
    async def batch_calculate(
        ...
        async with self.async_session_local() as session:
            ...
            if market:
                stocks, _ = await stock_repo.get_multi_with_filter(
                    session, market=market, limit=max_stocks
                )
            else:
                stocks, _ = await stock_repo.get_multi_with_filter(
                    session, limit=max_stocks
                )
            ...
            try:
                ...
                print(f"\n詳細結果:")
                print(
                    f"{'股票代號':<12} {'成功指標':<8} {'失敗指標':<8} {'執行時間':<10}"
                )
                print("-" * 45)
                ...
                for i, result in enumerate(results):
                    ...
                    print(
                        f"{stock_symbol:<12} {result.indicators_successful:<8} {result.indicators_failed:<8} {result.execution_time_seconds:<10.2f}"
                    )

            except Exception as e:
                print(f"批次計算失敗: {str(e)}")

    async def show_indicators(
        self, stock_id: int, indicator_types: str = None, days: int = 30
    ):
        """顯示指標數據"""
        async with self.async_session_local() as session:
            # ✅ Clean Architecture: 實例化 repository
            ...
            try:
                indicators_data = await technical_analysis_service.get_stock_indicators(
                    session, stock_id=stock_id, indicator_types=types_list, days=days
                )
        ...
        async with self.async_session_local() as session:
            ...
            try:
                ...
                price_data = await price_repo.get_stock_price_range(
                    session, stock_id=stock_id, start_date=start_date, end_date=end_date
                )
                    ...
                    data.append(
                        {
                            "date": price.date,
                            "open": (
                                float(price.open_price) if price.open_price else np.nan
                            ),
                            "high": (
                                float(price.high_price) if price.high_price else np.nan
                            ),
                            "low": (
                                float(price.low_price) if price.low_price else np.nan
                            ),
                            "close": (
                ...
                # 布林通道分析
                print(f"\n=== 布林通道分析 ===")
                bb_components = advanced_calculator.calculate_complete_bollinger_bands(
                    df
                )
                ...
                if recent_patterns:
                    ...
                    for pattern in sorted(
                        recent_patterns, key=lambda x: x["date"], reverse=True
                    )[:5]:
                        print(
                            f"  {pattern['date']}: {pattern['pattern']} (強度: {pattern['strength']})"
                        )
                ...
                if sr_levels["support_levels"]:
                    print(
                        f"支撐位: {[f'{level:.2f}' for level in sr_levels['support_levels'][-3:]]}"
                    )

                if sr_levels["resistance_levels"]:
                    print(
                        f"阻力位: {[f'{level:.2f}' for level in sr_levels['resistance_levels'][-3:]]}"
                    )
    ...
    async def export_indicators(self, stock_id: int, output_file: str, days: int = 90):
        ...
        async with self.async_session_local() as session:
            ...
            try:
                indicators_data = await technical_analysis_service.get_stock_indicators(
                    session, stock_id=stock_id, days=days
                )
...
async def main():
    ...
    calc_parser.add_argument(
        "--indicators", default="all", help='指標列表 (逗號分隔) 或 "all"'
    )
    ...
    batch_parser.add_argument(
        "--indicators", default="RSI,SMA_20,EMA_12", help="指標列表"
    )
