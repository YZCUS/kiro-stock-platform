...
from services.data.backfill import (
            ...
            for stock in stocks:
                try:
                    ...
                    if missing_dates:
                        ...
                        print(
                            f"{stock.symbol:<12} {stock.market:<6} {len(missing_dates):<10} {missing_ratio:.1%}"
                        )
    ...
    async def backfill_stock(
        ...
        async with self.async_session_local() as session:
            ...
            try:
                ...
                # 執行回補
                result = await data_backfill_service.execute_backfill_task(
                    session, task_id
                )
        ...
        async with self.async_session_local() as session:
            ...
            try:
                ...
                for result in results:
                    ...
                    task_display = (
                        "_".join(task_id_short)
                        if len(task_id_short) >= 2
                        else result.task_id[:30]
                    )

                    print(
                        f"{task_display:<30} {'✓' if result.success else '✗':<6} {result.dates_successful:<8} {result.execution_time_seconds:<10.2f}"
                    )
...
async def main():
    ...
    backfill_parser.add_argument(
        "--strategy",
        choices=["missing_only", "full_refresh", "incremental", "quality_based"],
