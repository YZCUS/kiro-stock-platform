                ...
                if result.success and result.signals_detected > 0:
                    ...
                    if high_confidence_signals:
                        ...
                        for signal in high_confidence_signals[:5]:  # 只顯示前5個
                            print(
                                f"      {signal.signal_type.value}: {signal.confidence:.2f} ({signal.date})"
                            )
    ...
    try:
        async with get_db() as db_session:
            ...
            if stock_symbols:
                ...
                for symbol in stock_symbols:
                    ...
                    if signals:
                        for signal in signals[:limit]:
                            print(
                                f"  {signal.date} | {signal.signal_type} | 信心度: {signal.confidence:.2f}"
                            )
                ...
                high_confidence_signals = (
                    await trading_signal_repo.get_high_confidence_signals(
                        db_session,
                        min_confidence=min_confidence,
                        days=days,
                        limit=limit,
                ...
                if high_confidence_signals:
                    ...
                    for stock_id, stock_signals in signals_by_stock.items():
                        ...
                        for signal in stock_signals:
                            print(
                                f"  {signal.date} | {signal.signal_type} | {signal.confidence:.2f}"
                            )
                            print(f"    {signal.description}")
                else:
                    print("沒有找到符合條件的信號")
...
async def signal_statistics_command(stock_symbols: List[str] = None, days: int = 30):
    """信號統計命令"""
            ...
            if stock_symbols:
                ...
                for symbol in stock_symbols:
                    ...
                    stats = await trading_signal_repo.get_signal_statistics(
                        db_session, stock_id=stock.id, days=days
                    )
                    ...
                    # 信號類型分布
                    distribution = (
                        await trading_signal_repo.get_signal_type_distribution(
                            db_session, stock_id=stock.id, days=days
                        )
                    )
...
async def signal_analysis_command(stock_symbol: str, signal_type: str, days: int = 90):
    """信號分析命令"""
    ...
    try:
        async with get_db() as db_session:
            ...
            performance = await trading_signal_repo.get_signal_performance_analysis(
                db_session, stock_id=stock.id, signal_type=signal_type, days=days
            )
...
async def cleanup_signals_command(
    keep_days: int = 365, stock_symbols: List[str] = None
):
    """清理舊信號命令"""
    ...
    try:
        async with get_db() as db_session:
            ...
            if stock_symbols:
                ...
                for symbol in stock_symbols:
                    ...
                    if stock:
                        deleted = await trading_signal_repo.delete_old_signals(
                            db_session, stock_id=stock.id, keep_days=keep_days
                        )
                ...
                # 清理所有股票的舊信號
                deleted = await trading_signal_repo.delete_old_signals(
                    db_session, keep_days=keep_days
                )
...
def main():
    ...
    detect_parser.add_argument(
        "--min-confidence", type=float, default=0.6, help="最小信心度"
    )
    ...
    list_parser.add_argument(
        "--min-confidence", type=float, default=0.0, help="最小信心度"
    )
    ...
    if args.command == "detect":
        ...
        asyncio.run(
            signal_statistics_command(stock_symbols=args.symbols, days=args.days)
        )

    elif args.command == "analyze":
        asyncio.run(
            signal_analysis_command(
                stock_symbol=args.symbol, signal_type=args.signal_type, days=args.days
            )
        )

    elif args.command == "cleanup":
        asyncio.run(
            cleanup_signals_command(
                keep_days=args.keep_days, stock_symbols=args.symbols
            )
        )
