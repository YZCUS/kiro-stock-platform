...
async def collect_all_stocks_data(
    ...
    try:
        ...
        return {
            ...
            "errors": [error for result in summary.results for error in result.errors][
                :50
            ],
        }
...
async def collect_batch_stocks_data(
    ...
    try:
        if request.use_stock_list and request.stocks:
            ...
            active_stocks = await data_collection_service.stock_repo.get_active_stocks(
                db
            )
            stock_entries = [
...
async def backfill_full_history(
    ...
    try:
        ...
        records_count = await data_collection_service.collect_stock_prices(
            db, stock_id=stock_id, period=period
        )
    ...
    try:
        ...
        for stock in active_stocks:
            try:
                # 檢查最新資料日期
                latest_price = (
                    await data_collection_service.price_repo.get_latest_price(
                        db, stock.id
                    )
                )
                ...
                records = await data_collection_service.collect_stock_prices(
                    db, stock_id=stock.id, days=backfill_days
                )
