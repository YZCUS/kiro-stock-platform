...
class TradingSignalRepository(ITradingSignalRepository):
    ...
    async def get_detailed_signal_stats(
        ...
        stats = await self.get_signal_statistics(
            db, stock_id=filters.get("stock_id") if filters else None
        )
        return stats
    ...
    async def get_stock_signals_by_date_range(
        ...
        query = query.order_by(
            TradingSignal.date.desc(), TradingSignal.confidence.desc()
        )
    ...
    async def get_high_confidence_signals(
        ...
        query = query.order_by(
            TradingSignal.confidence.desc(), TradingSignal.date.desc()
        ).limit(limit)
    ...
    async def get_signal_statistics(
        ...
        query = select(
            ...
            func.count(func.distinct(TradingSignal.signal_type)).label(
                "signal_types_count"
            ),
        ).where(and_(TradingSignal.date >= start_date, TradingSignal.date <= end_date))
        ...
        if row and row.total_count > 0:
            return {
                ...
                "max_confidence": (
                    float(row.max_confidence) if row.max_confidence else 0
                ),
                "min_confidence": (
                    float(row.min_confidence) if row.min_confidence else 0
                ),
    ...
    async def get_signal_type_distribution(
        ...
        query = (
            select(
                TradingSignal.signal_type, func.count(TradingSignal.id).label("count")
            )
            .where(
                and_(TradingSignal.date >= start_date, TradingSignal.date <= end_date)
            )
    ...
    async def get_signal_performance_analysis(
        ...
        return {
            ...
            "latest_signal_date": (
                max(s.date for s in signals).isoformat() if signals else None
            ),
            "earliest_signal_date": (
                min(s.date for s in signals).isoformat() if signals else None
            ),
        }
