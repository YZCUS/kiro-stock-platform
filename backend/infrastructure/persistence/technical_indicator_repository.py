...
class TechnicalIndicatorRepository(ITechnicalIndicatorRepository):
    ...
    async def get_by_stock_and_type(
        ...
        query = (
            query.order_by(desc(TechnicalIndicator.date)).offset(offset).limit(limit)
        )
    ...
    async def get_by_date_range(
        ...
        query = (
            query.order_by(
                desc(TechnicalIndicator.date), TechnicalIndicator.indicator_type
            )
            .offset(offset)
            .limit(limit)
        )
    ...
    async def get_latest_indicators(
        ...
        if indicator_types:
            subquery = subquery.where(
                TechnicalIndicator.indicator_type.in_(indicator_types)
            )
