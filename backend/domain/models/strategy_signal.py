...
class StrategySignal(BaseModel, TimestampMixin):
    ...
    confidence = Column(Numeric(5, 2), nullable=False, comment="信心度 (0-100)")
    entry_min = Column(Numeric(15, 4), nullable=False, comment="進場區間最小值")
    entry_max = Column(Numeric(15, 4), nullable=False, comment="進場區間最大值")
    stop_loss = Column(Numeric(15, 4), nullable=False, comment="停損價位")
    take_profit_targets = Column(JSON, nullable=False, comment="止盈目標（JSON 數組）")
    status = Column(
        String(20),
        nullable=False,
        default="active",
    ...
    extra_data = Column(JSON, nullable=True, comment="額外元數據（JSON格式）")

    # 約束條件和索引
    __table_args__ = (
        Index(
            "ix_strategy_signals_user_status_date", "user_id", "status", "signal_date"
        ),
        Index(
    ...
    def get_active_signals(
        cls, session, user_id: uuid.UUID, stock_id: Optional[int] = None
    ) -> List["StrategySignal"]:
        """獲取用戶的活躍信號"""
