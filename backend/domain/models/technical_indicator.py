...
class TechnicalIndicator(BaseModel, TimestampMixin):
    ...
    # 約束條件
    __table_args__ = (
        UniqueConstraint('stock_id', 'date', 'indicator_type', name='uq_technical_indicators_stock_id_date_indicator_type'),
        {'comment': '技術指標表'}
    )
    ...
    @classmethod
    def create_rsi_indicator(cls, stock_id: int, date: date, rsi_value: float, period: int = 14) -> 'TechnicalIndicator':
        """建立 RSI 指標"""
        return cls(
            stock_id=stock_id,
            date=date,
    ...
    @classmethod
    def create_sma_indicator(cls, stock_id: int, date: date, sma_value: float, period: int) -> 'TechnicalIndicator':
        """建立 SMA 指標"""
        return cls(
            stock_id=stock_id,
            date=date,
    ...
    @classmethod
    def create_ema_indicator(cls, stock_id: int, date: date, ema_value: float, period: int) -> 'TechnicalIndicator':
        """建立 EMA 指標"""
        return cls(
            stock_id=stock_id,
            date=date,
    ...
    @classmethod
    def create_macd_indicators(cls, stock_id: int, date: date, macd: float, signal: float, histogram: float, 
                              fast_period: int = 12, slow_period: int = 26, signal_period: int = 9) -> List['TechnicalIndicator']:
        """建立 MACD 相關指標"""
        parameters = {
    ...
    @classmethod
    def create_bollinger_bands(cls, stock_id: int, date: date, upper: float, middle: float, lower: float, 
                              period: int = 20, std_dev: float = 2.0) -> List['TechnicalIndicator']:
        """建立布林通道指標"""
        parameters = {
            'period': period,
            'std_dev': std_dev
    ...
    @classmethod
    def create_kd_indicators(cls, stock_id: int, date: date, k_value: float, d_value: float, 
                            k_period: int = 9, d_period: int = 3) -> List['TechnicalIndicator']:
        """建立 KD 指標"""
        parameters = {
            'k_period': k_period,
            'd_period': d_period
