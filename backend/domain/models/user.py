...
class User(BaseModel, TimestampMixin):
    ...
    email = Column(
        String(255), unique=True, nullable=False, index=True, comment="電子郵件"
    )
    username = Column(
        String(100), unique=True, nullable=False, index=True, comment="用戶名稱"
    )
    ...
    is_superuser = Column(
        Boolean, default=False, nullable=False, comment="是否為超級用戶"
    )
    ...
    @classmethod
    def create_user(
        cls, session, email: str, username: str, password: str, **kwargs
    ) -> "User":
        """建立新用戶"""
        hashed_password = cls.get_password_hash(password)
        user = cls(
            email=email, username=username, hashed_password=hashed_password, **kwargs
        )
        session.add(user)
        return user
