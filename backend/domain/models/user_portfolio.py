...
class UserPortfolio(BaseModel, TimestampMixin):
    ...
    avg_cost = Column(Numeric(20, 4), nullable=False, default=0, comment="平均成本（單價）")
    total_cost = Column(Numeric(20, 4), nullable=False, default=0, comment="總成本")

    # 約束條件
    __table_args__ = (
        UniqueConstraint('user_id', 'stock_id', name='uq_user_portfolios_user_id_stock_id'),
        {'comment': '用戶持倉表'}
    )
    ...
    def get_user_portfolio(cls, session, user_id: uuid.UUID) -> List['UserPortfolio']:
        ...
        return session.query(cls).options(joinedload(cls.stock)).filter(cls.user_id == user_id).all()

    @classmethod
    def get_portfolio_by_stock(cls, session, user_id: uuid.UUID, stock_id: int) -> Optional['UserPortfolio']:
        """取得用戶特定股票的持倉"""
    ...
    def create_or_update_position(
        ...
        if transaction_type == 'BUY':
            if portfolio:
                ...
                total_cost = (portfolio.avg_cost * portfolio.quantity) + (price * quantity)
    ...
    def calculate_profit_loss(self, current_price: Decimal) -> Dict[str, Decimal]:
        ...
        profit_loss_percent = (profit_loss / self.total_cost * 100) if self.total_cost > 0 else Decimal(0)
    ...
    def get_display_data(self, current_price: Optional[Decimal] = None) -> Dict[str, Any]:
        """取得顯示數據"""
        data = {
            'id': self.id,
