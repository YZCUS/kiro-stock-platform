...
class Transaction(BaseModel, TimestampMixin):
    ...
    portfolio_id = Column(Integer, ForeignKey("user_portfolios.id", ondelete="SET NULL"), nullable=True, index=True, comment="持倉ID（清倉後為 NULL）")
    stock_id = Column(Integer, ForeignKey("stocks.id", ondelete="CASCADE"), nullable=False, index=True, comment="股票ID")

    # 交易資訊
    transaction_type = Column(String(10), nullable=False, comment="交易類型（BUY/SELL）")
