...
class SystemLog(BaseModel):
    ...
    def debug(
        ...
        return cls.create_log(
            LogLevel.DEBUG, message, module, function_name, line_number, extra_data
        )
    ...
    def info(
        ...
        """建立 INFO 日誌"""
        return cls.create_log(
            LogLevel.INFO, message, module, function_name, line_number, extra_data
        )
    ...
    def warning(
        ...
        """建立 WARNING 日誌"""
        return cls.create_log(
            LogLevel.WARNING, message, module, function_name, line_number, extra_data
        )
    ...
    def error(
        ...
        """建立 ERROR 日誌"""
        return cls.create_log(
            LogLevel.ERROR, message, module, function_name, line_number, extra_data
        )
    ...
    def critical(
        ...
        """建立 CRITICAL 日誌"""
        return cls.create_log(
            LogLevel.CRITICAL, message, module, function_name, line_number, extra_data
        )

    @classmethod
    def get_logs_by_level(
        cls, session, level: str, limit: int = 100
    ) -> List["SystemLog"]:
        """根據等級取得日誌"""
        return (
            session.query(cls)
            .filter(cls.level == level.upper())
    ...
    def get_logs_by_module(
        cls, session, module: str, limit: int = 100
    ) -> List["SystemLog"]:
        """根據模組取得日誌"""
        return (
            session.query(cls)
    ...
    def get_error_logs(cls, session, limit: int = 100) -> List["SystemLog"]:
        ...
        return (
            session.query(cls)
            .filter(cls.level.in_(["ERROR", "CRITICAL"]))
            .order_by(cls.timestamp.desc())
            .limit(limit)
            .all()
        )
    ...
    def cleanup_old_logs(cls, session, days: int = 30) -> int:
        ...
        deleted_count = session.query(cls).filter(cls.timestamp < cutoff_date).delete()
