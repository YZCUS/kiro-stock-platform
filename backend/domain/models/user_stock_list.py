...
class UserStockList(BaseModel, TimestampMixin):
    ...
    is_default = Column(
        Boolean, default=False, nullable=False, comment="是否為預設清單"
    )
    sort_order = Column(
        Integer, default=0, nullable=False, comment="排序順序（數字越小越前面）"
    )
    ...
    list_items = relationship(
        "UserStockListItem", back_populates="stock_list", cascade="all, delete-orphan"
    )
    ...
    def get_user_lists(cls, session, user_id: uuid.UUID) -> List["UserStockList"]:
        ...
        return (
            ...
            .order_by(cls.sort_order, cls.is_default.desc(), cls.created_at)
            .all()
        )
    ...
    def get_default_list(cls, session, user_id: uuid.UUID) -> Optional["UserStockList"]:
        ...
        return (
            session.query(cls)
            .filter(cls.user_id == user_id, cls.is_default == True)
            .first()
        )
...
class UserStockListItem(BaseModel, TimestampMixin):
    ...
    def add_stock_to_list(
        cls, session, list_id: int, stock_id: int, note: Optional[str] = None
    ) -> "UserStockListItem":
        """添加股票到清單"""
        # 檢查是否已存在
        existing = (
            session.query(cls)
            .filter(cls.list_id == list_id, cls.stock_id == stock_id)
            .first()
        )
        ...
        item = cls(list_id=list_id, stock_id=stock_id, note=note)
        session.add(item)
        session.flush()
        return item
