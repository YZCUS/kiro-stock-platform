...
class UserStrategySubscription(BaseModel, TimestampMixin):
    ...
    monitor_all_lists = Column(
        Boolean, default=True, nullable=False, comment="是否監控所有清單"
    )
    ...
    def get_user_subscriptions(
        cls, session, user_id: uuid.UUID, active_only: bool = False
    ...
    def get_by_strategy_type(
        cls, session, user_id: uuid.UUID, strategy_type: str
    ) -> Optional["UserStrategySubscription"]:
        """根據策略類型獲取訂閱"""
        return (
            session.query(cls)
            .filter(cls.user_id == user_id, cls.strategy_type == strategy_type)
            .first()
        )
