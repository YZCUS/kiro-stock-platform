...
class TradingSignal(BaseModel, TimestampMixin):
    ...
    __table_args__ = (
        CheckConstraint(
            "confidence >= 0 AND confidence <= 1", name="ck_trading_signals_confidence"
        ),
        {"comment": "交易信號表"},
    )
    ...
    # 信號強度分級
    CONFIDENCE_LEVELS = {"LOW": (0.0, 0.4), "MEDIUM": (0.4, 0.7), "HIGH": (0.7, 1.0)}
    ...
    def get_display_data(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "direction": (
                "bullish"
                if self.is_bullish
                else "bearish" if self.is_bearish else "neutral"
            ),
    ...
    @classmethod
    def create_golden_cross_signal(
        cls,
        stock_id: int,
        date: date,
        price: float,
        ...
        """建立黃金交叉信號"""
        description = (
            f"短期均線({short_period}日)上穿長期均線({long_period}日)，形成黃金交叉"
        )
        return cls(
            stock_id=stock_id,
    ...
    def create_death_cross_signal(
        ...
        """建立死亡交叉信號"""
        description = (
            f"短期均線({short_period}日)下穿長期均線({long_period}日)，形成死亡交叉"
        )
        return cls(
            stock_id=stock_id,
