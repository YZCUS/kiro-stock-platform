...
def benchmark_single_indicator(
    indicator_name: str, calc_func, price_data: PriceData, iterations: int = 10
) -> Dict[str, Any]:
    """基準測試單個指標"""
    times = []
...
def benchmark_all_indicators(
    price_data: PriceData, iterations: int = 10
) -> List[Dict[str, Any]]:
    """基準測試所有指標"""
    print(
        f"開始基準測試 ({len(price_data.close_prices)} 天數據, {iterations} 次迭代)..."
    )
    ...
    indicators = [
        ...
        (
            "Williams_R",
            lambda: indicator_calculator.calculate_williams_r(price_data, 14),
        ),
...
def benchmark_batch_calculation(
    price_data: PriceData, iterations: int = 5
) -> Dict[str, Any]:
    """基準測試批次計算"""
    print("測試批次計算所有指標...")
...
def print_benchmark_results(
    benchmarks: List[Dict[str, Any]], batch_result: Dict[str, Any] = None
):
    """打印基準測試結果"""
    print("\n" + "=" * 80)
    ...
    print(
        f"{'指標名稱':<15} {'平均時間(ms)':<12} {'最小時間(ms)':<12} {'最大時間(ms)':<12} {'數據點數':<10}"
    )
    print("-" * 80)
    ...
    for benchmark in benchmarks:
        ...
        print(
            f"{benchmark['indicator']:<15} {avg_ms:<12.3f} {min_ms:<12.3f} {max_ms:<12.3f} {benchmark['data_points']:<10}"
        )
    ...
    if batch_result:
        ...
        batch_time_per_point = (
            (batch_result["avg_time"] * 1000) / total_data_points
