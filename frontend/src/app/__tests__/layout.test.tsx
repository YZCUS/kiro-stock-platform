...
describe("RootLayout", () => {
  ...
  it("應該渲染基本的佈局結構", () => {
    ...
    expect(screen.getByRole("complementary")).toBeInTheDocument(); // 桌面側欄
  });
  ...
  it("應該渲染所有導航連結", () => {
    ...
    // 檢查所有導航連結
    expect(
      screen.getByRole("link", { name: "股票分析平台" }),
    ).toBeInTheDocument();
    expect(screen.getByText("首頁")).toBeInTheDocument();
    expect(screen.getByText("即時分析")).toBeInTheDocument();
    expect(screen.getByText("系統狀態")).toBeInTheDocument();
    expect(screen.queryByText("股票工作區")).not.toBeInTheDocument();
    expect(screen.queryByText("策略與信號")).not.toBeInTheDocument();
  });
  ...
  it("導航連結應該有正確的 href 屬性", () => {
    ...
    expect(screen.getByText("即時分析").closest("a")).toHaveAttribute(
      "href",
      "/dashboard",
    );
    expect(screen.getByText("系統狀態").closest("a")).toHaveAttribute(
      "href",
      "/system",
    );
  });
  ...
  it("應該有響應式設計的 CSS classes", () => {
    ...
    const sidebar = screen
      ...
      .closest("aside");
    expect(sidebar).toHaveClass("fixed", "lg:flex");
