...
describe('StocksApiService', () => {
  ...
  it('maps stock CRUD and price endpoints', async () => {
    ...
    await StocksApiService.createStock({ symbol: 'AAPL', name: 'Apple', market: 'US' });
...
describe('stockListApi', () => {
  ...
  it('maps list and list item endpoints', async () => {
    ...
    expect(mockApiService.get).toHaveBeenNthCalledWith(1, '/api/v1/stock-lists/');
    expect(mockApiService.post).toHaveBeenNthCalledWith(1, '/api/v1/stock-lists/', {
      name: 'Watch',
      description: 'Tech',
    });
    ...
    expect(mockApiService.get).toHaveBeenNthCalledWith(2, '/api/v1/stock-lists/4/stocks');
    expect(mockApiService.post).toHaveBeenNthCalledWith(
      2,
      '/api/v1/stock-lists/4/stocks',
      { stock_id: 10 }
    );
    expect(mockApiService.delete).toHaveBeenCalledWith('/api/v1/stock-lists/4/stocks/10');
    expect(mockApiService.post).toHaveBeenNthCalledWith(
      3,
...
describe('stockValidationApi', () => {
  ...
  it('surfaces backend validation errors as user-facing errors', async () => {
    ...
    await expect(ensureStockExists('BAD', 'US')).rejects.toThrow('invalid symbol');
  });
});
...
describe('portfolioApi', () => {
  ...
  it('maps portfolio and transaction endpoints', async () => {
    ...
    expect(mockApiService.get).toHaveBeenNthCalledWith(2, '/api/v1/portfolio/summary');
...
describe('strategyApi', () => {
  ...
  it('maps strategy subscription endpoints with query params', async () => {
    ...
    expect(libApi.get).toHaveBeenNthCalledWith(1, '/api/v1/strategies/available');
    ...
    expect(libApi.put).toHaveBeenCalledWith('/api/v1/strategies/subscriptions/5', {
  ...
  it('maps strategy signal endpoints', async () => {
    ...
    expect(libApi.get).toHaveBeenNthCalledWith(1, '/api/v1/strategies/signals', {
      status: 'active',
      limit: 10,
    });
    expect(libApi.get).toHaveBeenNthCalledWith(
    ...
    expect(libApi.put).toHaveBeenCalledWith('/api/v1/strategies/signals/7/status', {
      status: 'triggered',
    });
